...
In order to increase your exposure to EON and offer liquidity to BitAssets, such
...
The Eidos network is capable of issuing any amount of any BitAsset and lend
it out to participants given enough collateral.

 * *settlement price*: The price for 1 EON as it is traded on external exchanges.
...
The Eidos network is capable of margin calling those positions that do not
...
offer to buy the collateral is less than the call price (x/EON).
...
market rates up to the SQP and close the position. The remaining EON of the
...
increase, an additional amount of EON is locked as collateral, while reducing
...
particular bitAsset to hand it over to the Eidos network. After that, the
