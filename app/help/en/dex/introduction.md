...
direct exchange of digital goods traded in the Eidos ecosystem.
...
briefly below. Although the Eidos DEX comes with all of them, it is up to
...
* **Separation of Powers**: 
  ...
  issuing IOUs and for processing the order book. In Eidos, order matching
  is performed by the protocol, which is unaware of implications concerning the
  involved assets.
  
* **Global Unified Order Book**:
  Since Eidos is global, anybody with an internet access can use the DEX for
  trading. This brings the world's liquidity to a single order book for
  decentralized trading.
  
* **Trade Almost Anything**:
  The Eidos DEX is asset agnostic. Hence you can trade at **any** pair.
...
* **No Limits**:
  The Eidos protocol is unable to limit your trading experience.
  
* **Decentralized**:
  The DEX is decentralized across the globe. This not only means that there is
  no single point of failure, but it also implies that the Eidos exchange is
...
* **Fast**:
  In contrast to other decentralized networks, the Eidos DEX allows for
  real-time trading and is only limited by the speed of light and the size of
  the planet.
  
* **Provable Order Matching Algorithm**:
  What makes the Eidos DEX unique is the provable order matching algorithm.
...
* **Collateralized Smartcoins**:
  One of the biggest features of Eidos are its *smartcoins* such as bitUSD,
  ...
  backed by Eidos' company shares (EON) being locked up as collateral and
