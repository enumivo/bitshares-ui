import {hash, key} from "eidosjs";

var eon_genesiskeys_bloom_url = undefined;
try {
    var url = require("file-loader?name=eon_genesiskeys_bloom_[sha1:hash:hex:7].dat!assets/eon_genesiskeys_bloom.dat");
    if (url.indexOf("3cee441") === -1)
        throw new Error("Incorrect hash: eon_genesiskeys_bloom.dat");
    eon_genesiskeys_bloom_url = url;
} catch (e) {
    // webpack deployment exception (not run time)
    console.log(
        "WARN: Will be unable to filter EON 1.0 wallet imports, did not find assets/eon_genesiskeys_bloom.dat",
        e
    );
}

/**
    This should only be applied to a EON 1.0 export file taken on the
    discontinued chain. Any public key string or address (all 5 formats) carried
    over to the EON 2.0 genesis block will be in this filter.
...
export default class GenesisFilter {
    ...
    isAvailable() {
        return eon_genesiskeys_bloom_url !== undefined;
    }
    ...
    init(done) {
        ...
        xhr.open("GET", eon_genesiskeys_bloom_url);
        xhr.send();
    }
    ...
    filter(account_keys, status) {
        if (!this.isAvailable()) {
            console.log("WARN: Missing bloom filter for EON 0.9.x wallets");
                ...
                for (var a = 0; a < account_keys.length; a++) {
                    ...
                    for (
                        ...
                        if (/^GPH/.test(currentKey))
                            currentKey = "EON" + currentKey.substring(3);
                        if (this.inGenesis(currentKey)) continue;
                        var addresses = key.addresses(currentKey, "EON");
